...
function mapToRow(item: ApiResultItem, index: number): ResultRow {
  const slug = (item.postSlug || "").trim();
  const title = (item.postTitle || "Untitled Result").trim();

  return {
    id: item.applicationId?.trim() || slug || `result-${index + 1}`,
    title,
    ...
    <main className="w-full py-3 sm:py-4">
      <section className="mx-auto w-[min(1220px,96vw)] space-y-2.5">
        <div className="relative overflow-hidden rounded-2xl border border-blue-200/80 bg-gradient-to-br from-blue-50 via-blue-100 to-indigo-100 p-3 shadow-[0_14px_30px_rgba(15,23,42,0.1)] ring-1 ring-blue-100/70 sm:p-4">
          ...
          <p className="relative text-[11px] font-black uppercase tracking-[0.14em] text-blue-800">Results</p>
          <h1 className="relative mt-1 text-xl font-black tracking-tight text-slate-900 sm:text-2xl">Fresh Published Results</h1>
