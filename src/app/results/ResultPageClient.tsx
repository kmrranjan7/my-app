      ...
      <section className="mx-auto w-[min(1220px,94vw)] space-y-2.5 sm:space-y-3">
        ...
        {rows.length === 0 ? (
          ...
          <section className="overflow-hidden rounded-2xl border border-cyan-100/90 bg-white/92 shadow-[0_14px_30px_rgba(15,23,42,0.1)]">
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/80 px-2.5 py-2 sm:px-3"><div><h2 className="text-[12px] font-black text-slate-900 sm:text-[13px]">Latest Updates</h2><p className="text-[9px] text-slate-500 sm:text-[10px]">Open an update to view the official result notice.</p></div><span className="rounded-full bg-indigo-50 px-2 py-0.5 text-[9px] font-bold text-indigo-700 sm:text-[10px]">{rows.length} updates</span></div>
                ...
                <tbody>
                  {rows.map((row, index) => (
                    <tr key={`${row.id}-${index}`} className="border-t border-slate-100 transition-colors hover:bg-cyan-50/50">
                      ...
                      <td className="px-2 py-2 align-top">
                        <div className="flex items-center justify-between gap-2"><ShareActionButton
                          title={row.title}
                          href={row.href}
                          contextLabel="Result"
                          details={[
                            ...
                            { label: "Released", value: row.startDate },
                          ]}
                        /><Link href={row.href} className="rounded-md border border-indigo-200 bg-indigo-50 px-2 py-1 text-[10px] font-bold text-indigo-700 transition-colors hover:border-indigo-300 hover:bg-indigo-100">View Details</Link></div>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {rows.map((row, index) => (
                <article key={`${row.id}-${index}`} className="rounded-xl border border-slate-200/90 bg-white p-2">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${row.startDate === "To Be Announced" ? "border-amber-200 bg-amber-50 text-amber-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>{row.startDate === "To Be Announced" ? "Pending" : "Result Released"}</span>
                  </div>
                  ...
                  <div className="mt-2 flex items-center justify-between gap-2"><ShareActionButton title={row.title} href={row.href} contextLabel="Result" details={[{ label: "Organization", value: row.badge }, { label: "State", value: row.state }, { label: "Released", value: row.startDate }]} showLabel={false} buttonClassName="inline-flex size-6 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 transition-colors hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700" iconClassName="size-3" copiedTextClassName="mt-1 text-[10px] font-semibold text-emerald-700" /><Link href={row.href} className="rounded-md border border-indigo-200 bg-indigo-50 px-2 py-1 text-[10px] font-bold text-indigo-700 transition-colors hover:border-indigo-300 hover:bg-indigo-100">View Details</Link></div>
                </article>
              ))}
            </div>
